...
class HaikuGenerator:
    def __init__(self, model_path: str, model_config: dict = None):
        ...
        if os.path.exists(model_path):
            ...
            print(
                f"Warning: Model checkpoint {model_path} not found. Using untrained model."
            )
    ...
    def interactive_mode(self):
        ...
        while True:
            try:
                user_input = input(
                    "\nEnter a prompt (or press Enter for random): "
                ).strip()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num_samples", type=int, default=1, help="Number of haikus to generate"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--evaluate", action="store_true", help="Evaluate model diversity"
    )
    parser.add_argument(
