...
VOCABULARY = list(string.ascii_letters + string.punctuation + string.digits + string.whitespace)
SPECIAL_TOKENS = ["<UNK>", "<PAD>", "<EOS>"]

#TODO: Check the way EOS is handled. The model seems to be predicting it as the first next character for every input.

class Tokenizer:
    def __init__(self, vocabulary: list = VOCABULARY, special_tokens: list = SPECIAL_TOKENS):
        vocabulary.sort()
        special_tokens.sort()
    ...
    def encode(self, text: str) -> list:
        ...
        encoded_text = [self.encode_dict.get(char, self.encode_dict["<UNK>"]) for char in text]
    ...
    def decode(self, encoded_text: list) -> str:
        ...
        decoded_text = "".join([self.decode_dict.get(idx, "<UNK>") for idx in encoded_text])
        return decoded_text
