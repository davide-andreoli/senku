...
from loaders.dataset import CSVListDataset, TextFolderDataset
...
def calculate_model_size(embedding_dim, context_length, num_layers, num_heads, vocab_size):
    ...
    feedforward = embedding_dim * (4 * embedding_dim) + (4 * embedding_dim) * embedding_dim
...
if __name__ == "__main__":
    ...
    parser.add_argument('--reset', action='store_true', help='Reset the model checkpoint')
    parser.add_argument('--dataset_type', choices=['csv', 'folder'], default='csv', help='Dataset type to use')
    parser.add_argument('--data_path', default='dataset/haiku/valid-haikus.csv', help='Path to dataset')

    parser.add_argument('-log', '--logging_level', default='info', help='Logging level (debug, info, warning, error)')
    parser.add_argument('--plot_curves', action='store_true', help='Plot training curves after training')
    ...
    parser.add_argument('--embedding_dimension', type=int, default=128, help='Embedding dimension')
    parser.add_argument('--context_length', type=int, default=64, help='Context length (shorter for haikus)')
    parser.add_argument('--num_layers', type=int, default=8, help='Number of transformer layers')
    parser.add_argument('--num_heads', type=int, default=8, help='Number of attention heads')
    parser.add_argument('--dropout', type=float, default=0.1, help='Dropout rate')
    parser.add_argument('--bias', action='store_true', help='Use bias in linear layers')

    parser.add_argument('--evaluation_frequency', type=int, default=100, help='Steps between evaluations')
    ...
    parser.add_argument('--max_length', type=int, default=50, help='Max sequence length for folder dataset')
    parser.add_argument('--stride', type=int, default=25, help='Stride for sliding window in folder dataset')
    ...
    dataset = CSVListDataset(file_path="dataset/haiku/valid-haikus.csv", tokenizer=tokenizer, context_length=args.context_length)
    train_dataloader, validation_dataloader = dataset.get_train_validation_loader(batch_size=32, num_workers=args.num_workers)
    ...
    save_model_config(model_config, os.path.join(args.checkpoint_dir, "model_config.json"))
    ...
    if args.train:
        trainer = Trainer(train_dataloader=train_dataloader, validation_dataloader=validation_dataloader, model=model, optimizer=optimizer, loss=loss_fn, checkpoint_path=checkpoint_path,
