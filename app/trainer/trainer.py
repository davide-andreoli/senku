...
class Trainer():
    ...
    def __init__(self, train_dataloader: DataLoader, validation_dataloader: DataLoader, model: nn.Module, optimizer: Optimizer, loss: nn.Module, device: torch.device = torch.device("cuda" if torch.cuda.is_available() else "cpu"), checkpoint_path: str = "checkpoints/checkpoint.pt", epoch: int = 0, reset_checkpoint: bool = False):
    ...
    def train(self, number_of_epochs: int, evaluation_frequency: int = 10, evaluation_mode: str = 'after_epoch'):

        train_losses = []
        validation_losses = []
        steps = 0
        epoch_progress_bar = tqdm(total=number_of_epochs, desc="Training Progress", unit="epoch")
        ...
        for epoch in range(self.epoch, self.epoch + number_of_epochs):
            ...
            batch_progress_bar = tqdm(total=len(self.train_dataloader), desc=f"Epoch {epoch + 1}/{number_of_epochs + self.epoch}", unit="batch")
            ...
            trainer_logger.info(f"Average training loss for epoch {epoch + 1}: {epoch_train_loss}")
